...
circleRadius = Math.round(
  Math.sqrt((canvisSize * (2827.43 / 3343113)) / Math.PI)
); //area at 0.08454% of total pixels
...
function determineAllPoints() {
  for (let i = 0; i < 361; i++) {
    xCord[i] = Math.round(
      circleRadius * Math.cos(i * (Math.PI / 180)) + player.x
    );
    yCord[i] = Math.round(
      circleRadius * Math.sin(i * (Math.PI / 180)) + player.y
    );
  }
}
...
function move() {
  ...
  for (let z = timeInterval; z < timeInterval + 20; z++) {
    //so in this case z will go from 1000 to 1020
